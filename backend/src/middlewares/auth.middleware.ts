...
import { getUserByEmailService, createUserService } from '../apis/services/user.service'; 
...
export async function isAuthenticated(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  // Development bypass: allow Bearer dev when enabled
  if (process.env.DEV_BYPASS_GOOGLE === 'true' && idToken === 'dev') {
    try {
      const devEmail = 'dev@example.com';
      const devName = 'Dev User';
      let user: IUser | null = await getUserByEmailService(devEmail);
      if (!user) {
        user = await createUserService(devEmail, devName, 'dev', '', undefined);
      }
      req.user = {
      ...
      res.status(500).json(errorResponse('Dev bypass failed.', 500));
      return;
    }
  }
