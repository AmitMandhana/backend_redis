...
export async function uploadCustomersController(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    if (!req.file) {
      res.status(400).json(error("No customer file uploaded.", 400));
      return;
    }
    ...
    readableStream
      .pipe(csvParser({
        mapHeaders: ({ header }) => (header ? header.toString().trim().toLowerCase() : header),
        strict: false,
        skipLines: 0,
        skipComments: false,
      }))
      ...
      .on('end', () => {
        if (errors.length > 0) {
          ...
          res.status(200).json(success({ message: "Customers file processed successfully.", count: results.length, results }, 200));
        }
      })
      .on('error', (err) => {
        console.error("CSV parsing error:", err);
        res.status(500).json(error("Error processing CSV file.", 500)); 
      });
  } catch (err) {
    console.error('Error in uploadCustomersController:', err);
    res.status(500).json(error("Internal server error", 500));
  }
}


export async function uploadOrdersController(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
        if (!req.file) {
            res.status(400).json(error("No order file uploaded.", 400));
            return;
        }
        if (!req.user || !req.user.id) {
            res.status(401).json(error("User not authenticated.", 401));
        ...
        readableStream
            ...
            .on('end', () => {
                if (errors.length > 0) {
                    ...
                    res.status(200).json(success({ message: "Orders file processed successfully.", count: results.length, results }, 200));
                }
            })
            .on('error', (err) => {
                console.error("CSV parsing error:", err);
        ...
        console.error('Error in uploadOrdersController:', err);
        res.status(500).json(error("Internal server error", 500));
    }
}
