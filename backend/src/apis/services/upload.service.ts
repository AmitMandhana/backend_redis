...
export async function processCustomerRow(row: CustomerCsvRow, userId: Types.ObjectId): Promise<ICustomer> {
  // normalize and trim
  const customer_external_id = (row.customer_external_id || (row as any)["external_id"] || (row as any)["externalid"])?.toString().trim();
  const name = (row.name || (row as any)["full_name"] || (row as any)["customer_name"])?.toString().trim();
  const email = (row.email || (row as any)["e-mail"])?.toString().trim();
